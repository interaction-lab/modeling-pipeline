...
class Windowing:
    ...
    def __init__(self, csv, config="./config/windowing_default.yml"):
        """[summary]
    ...
    def window_dataframe(self, window_size=30, step=10, keep_original=False):
...
def get_args():
    ...
    parser.add_argument(
        "input_path",
        help="where to find input csv file",
    )
    parser.add_argument("output_path", help="where to place the csv")
    parser.add_argument(
        "config",
        help="where to find input config file",
    )
